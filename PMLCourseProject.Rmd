...
Three different classifiers are trained by splitting the dataset 
into a training set (for training a model) and a validation set 
(for testing on new observations). The random forest classifier yields 
the best result with a 99.5% accuracy on the validation set. 
This model is then applied to a small testing set of 20 new observations, with 
an accuracy of 100%.
...
In [Section 1](#clean) the dataset is downloaded and cleaned. In 
[Section 2](#eda) an exploratory 
data analysis (EDA) is performed to get acquainted with the data and the 
options for modelling. 

Based on the EDA a set of preprocessing operations is performed in
[Section 3](#preproc). Different classifiers are trained and 
evaluated in [Section 4](#model), after which a final model is chosen 
...
errors. There appear to be many missing values (`NA`) as well as blanks and some erroneous values (`#DIV/0!`). Blanks and erroneous values are converted 
to `NA` after temporarily merging the training and test sets.
```{r replacevalues}
...
# remove the row number variable X
pmlData$X <- NULL
# replace some erroneous values by NA
pmlData[pmlData == ""] <- NA
pmlData[pmlData == "#DIV/0!"] <- NA
```
In the resulting dataset the total amount of missing values is calculated 
for each feature.
```{r missingvalues}
...
`train`) features there are missing values, varying between `r min(featMissing != 0)` and `r min(featMissing != 0)` observations or 98 to 100 percent.

Imputing all these missing values won't lead to good results
which is why all variables with missing values are removed from the dataset.
```{r removemissing}
# remove all features with missing values
...
`new_window` and `num_window`. The other irrelevant features are 
...
To explore the training data descriptive statistics (5-number summaries) and exploratory plots (boxplots and histograms) were generated for 
each of the `r ncol(pmlTrain2) -1` features. 
...
side-by-side boxplots (features vs. `classe`). 

This section describes the 3 main findings of the EDA.

## Normality of the features
Many of the **features don't follow a normal distribution** or can be easily normalized. The distributions in general are complex and multi-modal and 
...
The outlier has extreme values for 8 different variables.
```{r outlierdetail}
pmlTrain2[5373, c("total_accel_dumbbell", "gyros_dumbbell_x",
...
# Preprocessing <a id="preproc"></a>
The main conclusion of the EDA was that many of the features 
don't follow a normal distribution, which is why only machine learning
**algorithms which do not assume normality** will be considered. More 
specifically the tree-based algorithms (classification trees, random forests and gradient boosting) that don't require centering, scaling and/or 
normalization will be evaluated. 

The preprocessing therefore is limited to the **outlier removal** and **data slicing** of the training set.

## Outlier removal
The outlier on the 5373th row, identified during the EDA, 
is removed from the dataset.
```{r outlierremoval}
# remove outlier on 5373th row
pmlTrain2 <- pmlTrain2[-5373, ]
```

## Data slicing
The training set is split into a training set `training` (70% for training the classifier) and a validation set `validating` (30% for testing the model on new data).
```{r dataslicing}
# data slicing
set.seed(483)
...
evaluated: classification trees, random forests and gradient boosting. This 
section also includes a note about combining the models.

For evaluating the models two functions will be used:
```{r perffunctions}
...
(for example) by a 10-fold cross-validation on the training set `training`. A smaller `cp` value in general leads to more accurate but much less interpretable (more complex) models.
```{r tunecart, cache = TRUE}
set.seed(264)
...
than the default value. There is a significant increase in accuracy compared 
to the result with the default `cp` value. The tree plot below shows that 
the resulting tree is very hard to interpret (as expected).
```{r evalcart}
library(rpart.plot)
...
`n.minobsinnode` are kept fixed while a (limited) set of  values of `interaction.depth` and `n.trees` are used for tuning, again using 
5-fold cross-validation.
```{r tunegbm, cache = TRUE}
set.seed(264)
fitControl <- trainControl(method = "cv", number = 5)
...
greatly improve the results. Therefore it will not be investigated further in 
this analysis.

## Final model & prediction
The table below an overview of the training and validation set accuracies for the 
three different trained classifiers.
...
and is chosen as final model to predict the 20 different test set cases.
```{r applytest}
# predict with best (rf) model
predTestRF <- predict(trainRF, newdata = pmlTest2, type = "prob")
predTestRF$classe <- factor(apply(predTestRF, 1, which.max),
                            labels = LETTERS[1:5])
predTestRF
```